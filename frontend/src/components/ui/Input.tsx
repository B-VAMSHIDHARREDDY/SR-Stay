...
export interface InputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, "size"> {
  ...
  error?: string;
  icon?: ReactNode;
  /** Short fixed text rendered before the value, e.g. a country code ("+91"). */
  prefix?: string;
  size?: "sm" | "md" | "lg";
}
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative">
        {(icon || prefix) && (
          <span className="pointer-events-none absolute left-3.5 top-1/2 flex -translate-y-1/2 items-center gap-1.5 text-brand-black/40">
            {icon}
            {prefix && <span className="text-sm font-medium text-brand-black/60">{prefix}</span>}
          </span>
        )}
        <input
          ref={ref}
          ...
          className={cn(
            ...
            sizeClasses[size],
            icon && prefix ? "pl-16 pr-3.5" : icon || prefix ? "pl-10 pr-3.5" : "px-3.5",
            isPassword && "pr-10",
