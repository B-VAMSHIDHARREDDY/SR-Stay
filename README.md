...
- [ ] Replace placeholder social links, app store/Google Play URLs, and contact
      email in `src/lib/site-config.ts`
- [ ] Swap the inline text logo (`src/components/logo.tsx`) for the real SR
      Stays logo file if/when available
- [ ] Replace the generated favicon/OG image (`src/app/icon.tsx`,
      `src/app/opengraph-image.tsx`) with real brand assets if desired
- [ ] Update `siteConfig.url` in `src/lib/site-config.ts` to the final domain
      (used in canonical URLs, sitemap, JSON-LD)
- [ ] Wire up real listings data once a backend/database exists — search and
      city pages currently use static placeholder content
...
2. In the [Vercel dashboard](https://vercel.com/new), import the repo.
   Vercel auto-detects Next.js — no configuration needed (`vercel.json` in
   this repo just adds a few security headers and pins the framework).
3. Every push to `main` deploys to production automatically; every PR gets a
   preview deployment.
...
or want deploy logs in GitHub Actions.
...
1. In the Vercel dashboard, create a project linked to this repo **but
   ...
   avoid double deployments from both Vercel and this workflow.
2. Get a token: Vercel dashboard → Settings → Tokens.
3. Get your Org ID and Project ID: run `npx vercel link` locally once, then
